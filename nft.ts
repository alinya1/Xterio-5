...
class task {
    ...
    async chat() {
        try {
            let res = await this.axios({
                method: "POST",
                headers: {
                'Host': '3656kxpioifv7aumlcwe6zcqaa0eeiab.lambda-url.eu-central-1.on.aws',
                ...
                url: `https://3656kxpioifv7aumlcwe6zcqaa0eeiab.lambda-url.eu-central-1.on.aws/?address=${this.address}`,
                data: {"answer":"In a quaint village, Lily, known for her kindness, met Ethan, a lost wanderer. Their bond grew stronger each day as they shared dreams under the old oak tree. When a storm ravaged the village, their love inspired hope. Together, they rebuilt, showing that love conquers all."}
            });
            return res?.data;
        } catch (error) {
    ...
    async mint_nft() {
        try {
            ...
            console.log(`mint nft hash:${txResponse.hash}`);
            let txReceipt = await this.waitHash(txResponse.hash)
            if (txReceipt !== null) {
                console.log(`mint nft success hash:${txResponse.hash}`)
                // 提交hash
                try {
                    let result = await this.verify_txHash(txResponse.hash);
                    console.log(`mint nft success verify_txHash:${JSON.stringify(result)}`);
                } catch (error) {
                    console.log(`mint nft error verify_txHash:${error}`);
                }
                // 提交hash
            } else {
                console.log(`mint nft error hash:${txReceipt}`)
                fs.appendFileSync("mint_nft_error.txt", `${this.address}----${this.privateKey}----${this.id_token}\n`)
            }
        } catch (error) {
    ...
    async Run() {
        let chat = JSON.stringify(await this.chat());
        if (chat.includes('Wow') || chat.includes('repeated answers') ) {
            console.log(`send chat success:${chat}`)
        }else{
            console.log(`send chat error:${chat}}`)
            fs.appendFileSync("nft_error.txt", `${this.address}----${this.privateKey}----${this.id_token}\n`)
            return;
        }
